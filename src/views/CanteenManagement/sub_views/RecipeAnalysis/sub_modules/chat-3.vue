<template>
  <div class="chat-chunk">
    <div class="chat-title">学校食谱录入类型占比</div>
    <ElRow>
      <ElCol :span="14">
...
import { AddRecipeTypeList } from '@/global/const';
...
/** 初始化图表配置 */
const initChart = () => {
  const chartDOM = document.querySelector('.chart-3') as HTMLElement;
...
const getChartData = (array: Obj[]) => {
  ...
  if (array.length > 0) {
    for (const item of array) {
      seriesData.push({
        name: AddRecipeTypeList[item.type].name,
        value: item.count,
      });
    };
  } else {
