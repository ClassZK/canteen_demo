<template>
    <div class="layout-table" v-loading="tableModel.vLoading" element-loading-text="数据加载中">
        <div class="query-container">
          ...
          <IPlatformOrgFilter></IPlatformOrgFilter><ElFormItem label="督办主题" prop="subject">
                        <ElInput v-model="tableModel.query.subject" maxlength="30" show-word-limit clearable
                            placeholder="督办主题"></ElInput>
                    </ElFormItem>
                    <ElFormItem label="状态" prop="status">
                        <ElSelect v-model="tableModel.query.status" filterable clearable placeholder="状态">
                            <ElOption v-for="item of HandleStatusList" :key="item.value" :label="item.name"
        ...
        <IPlatformOrgColumn></IPlatformOrgColumn><ElTableColumn label="督办主题" prop="subject" min-width="150" align="center"
                    show-overflow-tooltip></ElTableColumn>
                <ElTableColumn label="督办人" prop="supervisor_name" min-width="150" align="center"
                    show-overflow-tooltip></ElTableColumn>
                <ElTableColumn label="督办时间" prop="supervise_time" min-width="160" align="center"
                    show-overflow-tooltip></ElTableColumn>
                <ElTableColumn label="状态" prop="status" width="150" align="center"
...
import { apiSupervisionList } from '@/api/supervision';
...
const onTableRequest = async () => {
    ...
    const { success, data, message } = await apiSupervisionList(tableModel.query);
    if (success) {
...
watch(() => SupervisionInspectionAuxStore.refresh, () => {
    onTableRequest();
});
