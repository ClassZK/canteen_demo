<template>
    <ElDialog width="1000px" title="自查项目详情" class="dialog-container" modal-class="dialog-overlay-custom"
        ...
        <div class="form-container" v-loading="formModel.vLoading" element-loading-text="数据加载中">
            <ElForm ref="formRef" :model="formModel.data" scroll-to-error disabled hide-required-asterisk label-width="80px" label-position="top">
                <ElRow :gutter="30">
                    <ElCol :span="12">
                        <ElFormItem label="自查项目" prop="item">
                            <ElInput v-model="formModel.data.item" placeholder=" "></ElInput>
                        </ElFormItem>
                    </ElCol>
                    <ElCol :span="12">
                        <ElFormItem label="自查结果" prop="status">
...
/** 监听操作类型 */
watch(() => SelfCheckEvaluationAuxStore.checkItemDetailData, (data) => {
    const object = JSON.parse(JSON.stringify(data));
    formModel.data = object;
}, {
    deep: true
});
watch(() => SelfCheckEvaluationAuxStore.checkItemDetailVisible, (boolean) => {
    if (boolean) {
