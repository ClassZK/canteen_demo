  ...
  <div class="layout-table" v-loading="tableModel.vLoading" element-loading-text="数据加载中">
    ...
    <div class="table-container">
      <ElTable height="100%" scrollbar-always-on :data="tableModel.data">
        ...
        <ElTableColumn label="入库参考价总金额(元)" prop="money" min-width="150" align="center" show-overflow-tooltip>
        </ElTableColumn>
        <ElTableColumn
          label="入库结算价总金额(元)"
          prop="guide_price"
...
import { apiInventoryInFoodStatisticsList } from "@/api/warehouse";
...
const onTableRequest = async () => {
  ...
  const { success, data, message } = await apiInventoryInFoodStatisticsList(tableModel.query);
  if (success) {
