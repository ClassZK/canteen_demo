<template>
  <div class="layout-table" v-loading="tableModel.vLoading" element-loading-text="数据加载中">
    <div class="query-container">
      <div class="query-left">
        <ElForm ref="formRef" :model="tableModel.query">
          ...
          <ElFormItem label="预警项" prop="warning_item">
            <ElSelect v-model="tableModel.query.warning_item" filterable clearable placeholder="预警项">
              <ElOption v-for="item in warningItems" :key="item" :label="item" :value="item"></ElOption>
            </ElSelect>
          </ElFormItem>
    ...
    <div class="table-container">
      <ElTable height="100%" scrollbar-always-on :data="tableModel.data">
        ...
        <ElTableColumn label="预警项" prop="warning_item" min-width="130" align="center" show-overflow-tooltip></ElTableColumn>
        <ElTableColumn label="摄像头" prop="camera_name" min-width="180" align="center" show-overflow-tooltip></ElTableColumn>
        <ElTableColumn label="抓拍时间" prop="snapshot_time" min-width="160" align="center" show-overflow-tooltip></ElTableColumn>
  ...
  <ElDialog v-model="dialog.visible" width="780px" :title="dialog.mode === 'handle' ? '视频预警处理' : '视频预警详情'" append-to-body draggable>
    <ElForm label-position="top">
      <ElRow :gutter="20">
        <ElCol :span="12"><ElFormItem label="预警项"><ElInput v-model="dialog.data.warning_item" disabled /></ElFormItem></ElCol>
        <ElCol :span="12"><ElFormItem label="摄像头"><ElInput v-model="dialog.data.camera_name" disabled /></ElFormItem></ElCol>
        <ElCol :span="12"><ElFormItem label="抓拍时间"><ElInput v-model="dialog.data.snapshot_time" disabled /></ElFormItem></ElCol>
        <ElCol :span="12"><ElFormItem label="状态"><ElInput :model-value="statusText(dialog.data.status)" disabled /></ElFormItem></ElCol>
        <ElCol><ElFormItem label="预警内容"><ElInput v-model="dialog.data.alarm_desc" type="textarea" :rows="4" disabled /></ElFormItem></ElCol>
        <ElCol><ElFormItem label="预警截图"><IUploadImage :data="dialog.data.alarm_files" disabled /></ElFormItem></ElCol>
...
const formRef = ref();
const warningItems = ["未规范着装", "未戴口罩", "人员离灶", "老鼠"];
const tableModel = reactive({
  vLoading: false,
  query: { page: 1, size: 20, warning_item: "", status: "", warn_category: "video" },
  total: 0,
  data: [] as Obj[],
});
...
const loadData = async () => {
  ...
  if (success) {
    let list = _utils.getDefaultArray(data.list).filter((item: Obj) => item.warn_category === "video");
    if (tableModel.query.warning_item) list = list.filter((item: Obj) => item.warning_item === tableModel.query.warning_item);
    tableModel.data = list;
    tableModel.total = list.length;
  } else {
