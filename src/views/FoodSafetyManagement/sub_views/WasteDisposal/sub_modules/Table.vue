      ...
      <ElTable height="100%" scrollbar-always-on :data="tableModel.data">
        ...
        >
          <template #default="scope">
            {{ onTableWasteTypeFilter(scope.row.waste_type) }}
          </template>
      </ElTableColumn>
        <ElTableColumn
          label="收运公司"
        ...
        <ElTableColumn
          label="收运商资质是否合格"
          prop="recipient_qualifications"
          min-width="150"
          align="center"
          show-overflow-tooltip
        >
          <template #default="scope">
            {{ onTableWhetherFilter(scope.row.recipient_qualifications) }}
          </template>
        </ElTableColumn>
...
import { apiWasteDisposalList } from "@/api/inspection";
...
const onTableWasteTypeFilter = (value: string) => {
  let text = "";
  const object = GarbageTypeList.find((item: Obj) => item.value === value);
  if (object) {
    return object.name;
  }
  return text;
