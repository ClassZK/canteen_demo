      ...
      <ElTable height="100%" scrollbar-always-on :data="tableModel.data">
        ...
        >
          <template #default="scope">
            {{ onTableMealTypeFilter(scope.row.meal_type) }}
          </template>
        </ElTableColumn>
        <ElTableColumn label="陪餐餐次" prop="accompanying_meals" min-width="150" align="center" show-overflow-tooltip>
          <template #default="scope">
            {{ onTableMealTimeFilter(scope.row.accompanying_meals) }}
          </template>
        </ElTableColumn>
        <ElTableColumn
          label="陪餐人"
...
import { OperationTypeEnum, Message, MealtimeList, MealAccompanyList } from "@/global/const";
import _utils from "@/utils/index";
import { apiMealAccompanimentList, apiMealAccompanimentDelete } from "@/api/inspection";
...
const onTableRequest = async () => {
  ...
  const { success, data, message } = await apiMealAccompanimentList(tableModel.query);
  if (success) {
...
const onTableDelete = (data: Obj) => {
  ElMessageBox.alert(`确定删除陪餐记录吗？`, "温馨提示", {
    ...
    beforeClose: async (action, instance, done) => {
      if (action === "confirm") {
        ...
        const { success, message } = await apiMealAccompanimentDelete({
          id: data.id,
        });
        if (success) {
          done();
          onTableRequest();
          Message.success(` 陪餐记录删除成功`);
        } else {
          Message.warning(message);
        }
...
const onTableMealTypeFilter = (value: string) => {
  let text = "";
  const object = MealAccompanyList.find((item: Obj) => item.value === value);
  if (object) {
    return object.name;
  }
  return text;
}
const onTableMealTimeFilter = (value: string) => {
  let text = "";
  const object = MealtimeList.find((item: Obj) => item.value === value);
  if (object) {
    return object.name;
  }
  return text;
...
watch(
  () => MealAccompanimentAuxStore.refresh,
  () => {
    onTableRequest();
  }
);
